...
const App = ({
    ...
    const editPointsCollection=async ()=>{ 
 
        let payload={...pointsCollection[0]}
    ...
    const createPointsCollection=async ()=>{ 
 
        let payload={
            "branchId": bhid,
            "brandGroupId": customerGroups[0].id,
            "amount": 10,
            "rewardProgramGifts": [],
            "startDate":new Date(),
        ...
        await  branchService.createPointsCollectionProgram(payload) 
        .then((res)=>{  
            // setReload(prev => !prev) 
        })
        .catch((err)=>{
        ...
        ()=>{
            getCustomergroups()  
             if(pointsCollection.length>0){
                setData({
                    ...data,
                    amountPurchaseReward:pointsCollection[0]?.amount,
                    giftPrograms:pointsCollection[0]?.rewardProgramGifts,
                    groupOfCustomers:pointsCollection[0]?.brandGroupId
        ...
        <Grid container spacing={4}>
            ...
            <Grid item xs={12}>
                <Grid container spacing={2} >
                    <Grid item xs={3}>
                        <Counter title="Set amount of points equals 1 Currency Value i.e 1 currency unit = 2 points"
            ...
            <Grid item xs={12}>
                <Grid container >
                    ...
                    >
                        ...
                        <Grid item>
                            <Button variant="outlined" color="error" onClick={()=>deletePointsCollection(pointsCollection[0]?.id)}>delete</Button>
                        </Grid>
                        <Grid item>
                            <Button primay variant="contained" onClick={editPointsCollection} >Save</Button>
                        </Grid>
                        ...
                        <Grid item xs={12}>
                            ...
                            onClick={createPointsCollection}
                            >
                                Create new
                            </Button>
                            </Grid>
