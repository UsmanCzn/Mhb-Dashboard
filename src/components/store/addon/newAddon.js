...
useEffect(
  ...
  const createNewType=async (event)=>{
    ...
    await fileService.uploadProductImage(p1)
    .then((res)=>{ 
    ...
    await storeServices.createProductAddition(payload)
...
image:""
    ...
    await storeServices.updateProductAddition(payload,selectedBrand?.id)
  ...
  const deleteAddon =async  ()=>{
    await storeServices.deleteProductAddon(updateData?.id,selectedBrand?.id)
