        ...
        const handleClose = (data) => {
            ...
            if (data.name == 'Accept') {
                const body = {
                    ...
                    increasePunchesCount: selectedRow.increasePunchesCount,
                    pointsUsed: 0,
                    pointsEarned: 0,
                    comments: ''
                };
...
const columns = [
  ...
  {
    field: 'increasePunchesCount',
    headerName: 'Stamps Request',
    flex: 0.9,
    minWidth: 140,
    headerAlign: 'right',
    align: 'right',
    ...
    renderCell: (params) =>
      !params.row.isAct && (
        ...
        return (
            <>
                <DataGridComponent
                    rows={freeDrinksRequest.filter((e) => e.increasePunchesCount > 0)}
                    columns={columns}
                    loading={false}
                    getRowId={(row) => row.id}
                    pMode="client"
                    rowsPerPageOptions={[10]}
                    totalRowCount={freeDrinksRequest.filter((e) => e.increasePunchesCount > 0)?.length}
